...
class Reminders():
    def __init__(self, app):
        ...
        print(">init checking 3")
        self.app = app
        self.reminders = self.lists = {}
        print(">init checking4444")
    ...
    def _remote_remove_task(self, user_id, task_list, task_id):
        if user_id in self.caldav.users.keys():
                ...
                with open(LISTS_FILE, newline='') as csvfile:
                    ...
                    for row in reader:
                        try:
                            list_id = row['id']
                            user_id = row['user-id']
                            if user_id not in 'local' and user_id not in self.caldav.users.keys():
                                continue
                            lists[list_id] = {
                                'name': row['name'],
                                'user-id': user_id,
    ...
    def _to_remote_task(self, reminder, location, updating, old_user_id = None, old_list_id = None, old_task_id = None, completed = None, completed_timestamp = None, completed_date = None):
        ...
        if moving:
            if old_user_id is None:
                old_user_id = user_id
            if old_user_id in self.caldav.users.keys():
    ...
    def create_list(self, app_id, variant = True, **kwargs):
        ...
        if user_id in self.caldav.users.keys() or user_id == 'local':
